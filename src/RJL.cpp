...
real Simi_upgrade(int u, int i, int j, int k,real ri,real rj, real rk) {
	p = pu + u*dim;
	q = qi + i*dim;
	qj = qi + j*dim;
	qk = qi + k*dim;
...
void train() {
	...
	for (int local_iter = 0; local_iter < iter; local_iter++) {
		...
		while (!feof(fp))
		{
			...
			fgets(buf, 1024, fp);
			sscanf(buf, "%d,%d,%d,%d,%f,%f,%f", &u, &i, &j, &k,&tri,&trj,&trk);
			// 
			ri= (real) tri;
			rj=(real) trj;
			rk = (real) trk;
			// printf("%s\n",buf);
			// printf("%f,%f,%f",ri,rj,rk);
			// exit(0);
